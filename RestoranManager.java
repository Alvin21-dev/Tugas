...
class Diskon extends MenuItem {
    ...
    @Override
    public void tampilMenu() {
        System.out.println("Promo: " + getNama() + " | Harga: " + getHarga() + " | Kategori: " + getKategori() + " | Diskon: " + (diskon * 100) + "%");
    }
}
...
class Menu {
    ...
    public void tampilMenu() {
        ...
        // Sort daftarMenu by kategori before displaying
...
class Pesanan {
    ...
    public double hitungTotal() {
        ...
        for (MenuItem item : daftarPesanan) {
            ...
            if (item instanceof Diskon) {
                diskonTotal += ((Diskon) item).getDiskon() * total; // Diskon diterapkan pada total
            }
        }
    ...
    public void muatStrukDariFile(String namaFile) {
        // Implementasi sederhana, asumsikan format sama dengan simpan
        try (BufferedReader reader = new BufferedReader(new FileReader(namaFile))) {
            String line;
            while ((line = reader.readLine()) != null) {
                if (line.startsWith("Makanan:")) {
                    // Parse dan tambah, tapi untuk kesederhanaan, skip detail
...
public class RestoranManager {
    ...
    private static int strukCounter = 1; // Added counter for automatic file naming
    ...
    private static void tambahItemMenu() {
        ...
        if (jenis == 1) {
            System.out.print("Kategori: Makanan");
            String kategori = "Makanan";
            menu.tambahItem(new Makanan(nama, harga, kategori));
        } else if (jenis == 2) {
            System.out.print("Kategori: Minuman");
    ...
    private static void terimaPesanan() {
        ...
        if (simpan.equalsIgnoreCase("y")) {
            String namaFile = "struk" + strukCounter + ".txt"; // Auto-generated filename
    ...
    private static void lihatStrukPesanan() {
        ...
        int pilihanFile = -1;
        try {
            pilihanFile = Integer.parseInt(scanner.nextLine());
